...
isTokenValid = () => {
    ...
    try {
        ...
        console.log("Error retrieving token from : " + configs.TOKEN_PATH + ", please wait while we get a new token.");
        return false
    }

    if (token.expires_on == "") {
        console.log("Error while retrieving token expiry date from : "  + configs.TOKEN_PATH)
        return false;
    }
...
getToken = async () => {
    ...
    if (isTokenValid()) {
        ...
        return await request(options).then(
            result => {
                writeFile(configs.TOKEN_PATH, result, () => {
